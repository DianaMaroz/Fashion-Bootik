...
async def navi_goods(call: CallbackQuery):
    ...
    for product in db_var.values():
        if product.get('type') == current_item and product.get('id') == current_id:
            cur_product = product
    print(cur_product)
    photo = InputFile(path_or_bytesio=cur_product[current_id-1][2])
    name = call.message.from_user.full_name
    current_chat_id = call.message.chat.id
    current_message_id = call.message.message_id
    caption = f"{cur_product[current_id-1][3]}\n{cur_product[current_id-1][4]}\n\n" \
              f"Размеры: {cur_product[current_id-1][5]}\nСтоимость: {cur_product[current_id-1][7]}"
