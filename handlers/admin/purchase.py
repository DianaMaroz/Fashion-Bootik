...
PICKUP_SHIPPING_OPTION.add(LabeledPrice(label='Самовывоз', amount=000))
...
async def shipping_option(shipping_query: ShippingQuery):
    shipping_options = [EXPRESS_SHIPPING_OPTION, PR_SHIPPING_OPTION, PICKUP_SHIPPING_OPTION]
    await dp.bot.answer_shipping_query(shipping_query.id, ok=True, shipping_options=shipping_options)
